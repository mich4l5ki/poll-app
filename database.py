...
SELECT_LATEST_POLL = "SELECT * FROM polls WHERE polls.id = (SELECT id FROM polls ORDER BY id DESC LIMIT 1);"
...
INSERT_VOTE = "INSERT INTO votes (username, vote_timestamp, option_id) VALUES (%s, %s, %s);"
...
def add_poll_vote(connection, username: str, vote_timestamp: float, option_id: int) -> None:
    with get_cursor(connection) as cursor:
        cursor.execute(INSERT_VOTE, (username, vote_timestamp, option_id))
